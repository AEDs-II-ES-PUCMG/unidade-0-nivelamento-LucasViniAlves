...
public class Pedido {
	...
	private ItemDePedido[] itensDePed;
	...
	/** Construtor do novo pedido.
	 *  Deve criar o vetor de produtos do pedido, 
	 *  armazenar a data e a forma de pagamento informadas para o pedido. 
	 */  
	public Pedido(int quantidade, ItemDePedido itens, double precoVenda) {
		
		itens = new ItemDePedido(produtos, quantidade, precoVenda);
		quantProdutos = 0;
		precoVenda = 0;
	}
	
	/**
     * Inclui um itemDePedido neste pedido e aumenta a quantidade de produtos armazenados no pedido até o momento.
     ...
     */
	public boolean incluirItemPedido(ItemDePedido novo) {
		
		if (quantProdutos < MAX_PRODUTOS) {
			
			itensDePed[quantProdutos++] = novo;
			return true;
		}
		return false;
	}
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantProdutos; i++) {
			valorPedido += produtos[i].valorVenda();
		}
	...
	public void mesclarPedido(Pedido outroPedido){
		...
		int soma =  quantProdutos + outroPedido.quantProdutos;

	    if (soma < MAX_PRODUTOS)
			throw new IllegalArgumentException("Total produtos maior que o maximo");
			
			for (int i = 0; i < soma; i++){
				
				if (itensDePed[i].equals(outroPedido))
					return;
				}

				itensDePed[quantProdutos] = outroPedido;
